...
export class UsersComponent implements OnInit {
  ...
  displayedColumns: string[] = ['nom', 'prenom', 'email', 'role','toggleStatus','update', 'Delete'];
...
/*toggleUserStatus(userId: string, currentStatus: boolean) {
  ...
  const confirmation = window.confirm(
    `Are you sure you want to ${newStatus ? 'unlock' : 'lock'} this user?`
  );

  if (confirmation) {
    // Si l'utilisateur a confirmé, mettez à jour le statut ici
    this.userService.updateUserStatus(userId, newStatus).subscribe(
      () => {
        // Mettez à jour la liste des utilisateurs ou effectuez d'autres actions nécessaires
        this.loadUsers();
      },
      (error) => {
